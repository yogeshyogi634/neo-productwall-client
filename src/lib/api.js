...
export const api = {
  ...
  changeUpdateStatus: (id, status, reason = null) =>
    request(`/updates/${id}/status`, {
      method: "PATCH",
  ...
  getFeedbackDateRange: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return request(
      `/feedback/date-range${queryString ? `?${queryString}` : ""}`,
    );
  },
