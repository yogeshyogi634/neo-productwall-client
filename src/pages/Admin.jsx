...
export default function AdminPage() {
  ...
  const checkAdminAccess = async () => {
    try {
      ...
      // ADMIN CHECK: Master admin (madhav) always allowed, or any user with ADMIN role
      const MASTER_ADMIN = "madhav@neokred.tech";
      const isMasterAdmin = user.email === MASTER_ADMIN;
      const isAdminRole = user.role?.toUpperCase() === "ADMIN";

      // Allow access if: master admin OR has ADMIN role
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-semibold text-text-default-primary">
                Admin Panel
              </h1>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <StatCard
            ...
            value={
              allUsers.filter(
                (u) => u.role === "MANAGEMENT" || u.role === "ADMIN",
              ).length
            }
          ...
          <div className="space-y-3">
            {pendingUsers.length === 0 ? (
              <EmptyState
                ...
                description="All signups have been reviewed."
              />
            ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-stroke-default-primary">
                  <th className="text-left px-6 py-3 text-xs font-medium text-text-default-secondary uppercase tracking-wider">
                    User
                  </th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-text-default-secondary uppercase tracking-wider">
                    Designation
                  ...
                  <th className="text-right px-6 py-3 text-xs font-medium text-text-default-secondary uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-stroke-default-primary">
                {allUsers.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-6 py-12 text-center text-text-default-secondary text-sm"
                    >
              ...
              <tbody className="divide-y divide-stroke-default-primary">
                {logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <tr
                      key={log.id}
                      className="hover:bg-background-card-secondary/50 transition-colors"
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-text-default-secondary">
                        {log.targetUser ? (
                          <div>
                            <p className="text-text-default-primary">
                              {log.targetUser.name}
                            </p>
    ...
    <div className="bg-background-card-primary border border-stroke-default-primary rounded-xl p-5 flex items-center justify-between gap-4">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-text-default-secondary text-sm truncate">
            {user.email}
          </p>
          <p className="text-text-default-secondary text-xs mt-0.5">
            {user.department}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-text-default-secondary">
            Assign role:
          </label>
          <select
            value={selectedRole}
...
function UserRow({ user, isSelf, loading, onRoleChange, onRemove }) {
  ...
  return (
    <tr
      className={`hover:bg-background-card-secondary/50 transition-colors ${isSelf ? "bg-brand-primary/5" : ""}`}
    >
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <p className="text-text-default-primary text-sm font-medium">
                {user.name}
              </p>
              {isSelf && (
      ...
      <td className="px-6 py-4">
        {isAdmin || isSelf ? (
          <span
            className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${currentRole.color}`}
          >
            {currentRole.label}
          </span>
        ) : (
